...
class Sioyek:
    ...
    def run_command(self, command_name, text=None, focus=False):
        if text == None:
            ...
            params = [self.path, '--execute-command',
                      command_name, '--execute-command-data', str(text)]
    ...
    def open_document_embedded_from_current_path(self, focus=False):
        self.run_command(
            "open_document_embedded_from_current_path", None, focus=focus)
...
class Highlight:
    ...
    def get_end_document_pos(self):
        end_page, end_offset_y = self.doc.absolute_to_document_y(
...
class Bookmark:
    ...
    def insert(self, document):
        ...
        cursor = self.doc.sioyek.shared_database.cursor()
        cursor.execute(INSERT_QUERY, (document_hash,
                       self.description, self.y_offset))
        cursor.close()
...
class Document:
    ...
    def to_absolute(self, document_pos):
        ...
        offset_y = document_pos.offset_y + \
            self.cum_page_heights[document_pos.page]
    ...
    def to_document(self, absolute_document_pos, pypdf=False):
        page, offset_y = self.absolute_to_document_y(
            absolute_document_pos.offset_y)
        if pypdf:
    ...
    def embed_highlight(self, highlight, colormap=None):
        ...
        if method == 'fitz':
            ...
            selected_rects = [fitz.Rect(*word[:4])
                              for word in selected_words[0]]
    ...
    def add_imported_bookmark(self, page, bookmark):
        ...
        new_bookmark = Bookmark(
            self, bookmark.info['content'], absolute_pos.offset_y)
    ...
    def import_annotations(self, colormap=None):
        ...
        for page, text, hl in new_highlights:
            ...
            if colormap:
                highlight_type = find_highlight_type_with_color(
                    color, colormap)
            else:
                highlight_type = 'a'
            ...
            begin_pos = (begin_rect[0][0],
                         (begin_rect[0][1] + begin_rect[2][1]) / 2)
            end_pos = (end_rect[1][0], (end_rect[0][1] + end_rect[2][1]) / 2)
            self.add_imported_highlight(
                page, begin_pos, end_pos, text, highlight_type)
            ...
            for pdf_hl in pdf_highlights:
                ...
                if not found:
                    new_highlights.append(
                        (page_number, pdf_highlight_text, pdf_hl))
    ...
    def get_non_embedded_highlights(self):
        ...
        for highlight in candidate_highlights:
            ...
            for pdf_highlight in pdf_page_highlights:
                ...
                highlight_text = get_pdf_highlight_text(
                    pdf_highlight, document_page)
    ...
    def get_page_text_and_rects(self, page_number):
        ...
        for i in range(len(word_data)):
            ...
            string_rects.extend(
                [fitz.Rect(word_data[i][0:4])] * len(additional_string))
    ...
    def get_best_selection_rects(self, page_number, text, merge=False):
        for i in range(10):
            rects = self.get_text_selection_rects(
                page_number, text, num_errors=i)
            if len(rects) > 0:
    ...
    def get_best_selection(self, page_number, text):
        for i in range(10):
            res = self.get_text_selection_begin_and_end(
                page_number, text, num_errors=i)
            if res[0][0] != None:
                return res
        return None
    ...
    def get_text_selection_rects(self, page_number, text, num_errors=0):
        if num_errors == 0:
            ...
            page_text, page_rects, _, _ = self.get_page_text_and_rects(
                page_number)
            match = regex.search(
    ...
    def get_bookmarks(self):
        ...
        BOOKMARK_SELECT_QUERY = "select * from bookmarks where document_path='{}'".format(
            doc_hash)
        shared_database = self.sioyek.get_shared_database()
        cursor = shared_database.execute(BOOKMARK_SELECT_QUERY)
        bookmarks = [Bookmark(self, desc, y_offset)
                     for _, _, desc, y_offset in cursor.fetchall()]
        return bookmarks

    def get_highlights(self):
        doc_hash = self.get_hash()
        HIGHLIGHT_SELECT_QUERY = "select * from highlights where document_path='{}'".format(
            doc_hash)

        shared_database = self.sioyek.get_shared_database()
        cursor = shared_database.execute(HIGHLIGHT_SELECT_QUERY)
        results = cursor.fetchall()
        highlights = [Highlight(self, text, highlight_type, (begin_x, begin_y), (end_x, end_y)) for _, _, text,
                      _, highlight_type, _, _, _, begin_x, begin_y, end_x, end_y in results]
        print(highlights)

        return highlights

    def get_highlights_with_time_stamps(self):
        doc_hash = self.get_hash()
        ...
        results = cursor.fetchall()
        # Sory by Begin-y
        results = sorted(results, key=lambda x: x[9])
        return results
    ...
    def get_highlight_bounding_box(self, selection_begin, selection_end):
        ...
        words, page_number = self.get_selected_words(
